...
namespace Template
{
    ...
    class Scene
    {
        ...
        static Sphere plane1, plane2; // very large spheres approximating planes
        ...
        public Material GetMaterial( int objIdx, Vector3 I )
        {
            ...
            { mat.refl = mat.refr = 0; mat.emissive = false; mat.diffuse = Vector3.One; }
            if ( objIdx == 2 )
            { mat.refl = 0.8f; mat.refr = 0; mat.emissive = false; mat.diffuse = new Vector3( 1, 0.2f, 0.2f ); }
            if ( objIdx == 3 )
            { mat.refl = 0; mat.refr = 1; mat.emissive = false; mat.diffuse = new Vector3( 0.9f, 1.0f, 0.9f ); }
            if ( objIdx == 4 )
            { mat.refl = 0.8f; mat.refr = 0; mat.emissive = false; mat.diffuse = new Vector3( 0.2f, 0.2f, 1 ); }
            if ( (objIdx > 4) && (objIdx < 8) )
            { mat.refl = mat.refr = 0; mat.emissive = false; mat.diffuse = Vector3.One; }
            if ( objIdx == 8 )
            { mat.refl = mat.refr = 0; mat.emissive = true; mat.diffuse = lightColor; }
            return mat;
        }
